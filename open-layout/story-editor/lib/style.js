 ...
 * @typedef {{ bold?: boolean, italic?: boolean, fontFamily?: string, [key: string]: unknown }} CharacterStyle
 ...
 */
export const DEFAULT_STYLE = Object.freeze({ bold: false, italic: false, fontFamily: '' });
...
export function styleEq(a, b) {
  ...
  if ((sa.fontFamily || '') !== (sb.fontFamily || '')) return false;

  const keysA = Object.keys(sa);
  for (const key of keysA) {
    if (key === 'bold' || key === 'italic' || key === 'fontFamily') continue;
