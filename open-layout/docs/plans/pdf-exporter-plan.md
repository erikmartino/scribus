...
#### [RENAME/MODIFY] `doc-renderer/lib/render-document.js` (formerly `svg-generator.js`)
Acts as the central orchestrator that resolves the document layout and story data.

#### [NEW] `doc-renderer/lib/svg-renderer.js`
Extract the SVG generation logic out of the old `svg-generator.js` into a dedicated SVG backend. This module generates the SVG nodes from the document layout.
- **No Duplicate Styling Logic**: All styling resolution (e.g., font family, weight, italics, colors) must be performed exactly once by `doc-renderer` and applied as explicit attributes on the SVG nodes generated by `svg-renderer`. The `pdf-exporter` must rely purely on reading these computed SVG attributes from the SVG DOM, avoiding any duplicated styling interpretation logic.
...
High-level PDF backend. Imports `renderDocument` from the `doc-renderer` module and `PdfWriter`.

- `streamDocument(engine, docPath, opts)` → `ReadableStream`.
- Per spread/page loop:
  1. Call `renderDocument()` to get the rendered SVG pages.
  2. Extract font subsetting data directly from the rendered SVG text elements.
  3. Fetch each image URL (only images on this page, sequentially).
  4. Write image XObjects to stream → let GC collect the data.
  5. Walk the SVG DOM (`<text>`, `<tspan>`, `<image>`), emitting corresponding PDF operators.
