...
describe('buildPositions', () => {
  ...
  it('adds right-edge entry with next-char charPos for hyphenation break', () => {
    const entry = makeEntry({
    ...
    // Right-edge entry uses hyphToOrig[endChar] = 2 (the next original char)
    assert.equal(secondLast.charPos, 1); // 'b'
    assert.equal(last.charPos, 2);       // next char, not a duplicate
    assert.ok(last.x > secondLast.x);
  });
});
