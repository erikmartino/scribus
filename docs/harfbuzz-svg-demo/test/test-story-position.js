...
describe('positionToPoint', () => {
  ...
  it('returns right-edge x for charOffset past last position (hyphenated line)', () => {
    const lineMap = [makeLine(0, 0, [
      { charPos: 0, x: 16 }, { charPos: 1, x: 26 },
      { charPos: 1, x: 40 }, // right-edge duplicate
    ])];
    // charOffset 1 should return the LAST match (right edge)
    const pt = positionToPoint({ paraIndex: 0, charOffset: 1, lineIndex: 0 }, lineMap, 20);
    assert.equal(pt.x, 40);
  });
});
